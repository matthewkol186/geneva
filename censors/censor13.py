"""
Censor 13 is a RST censor designed to more closely mimic GFW behavior. It
...
that are incorrect). It also resynchronizes on both S and ACK, defeating strategies
that trigger before the 3-way handshake has finished. This censor more closely
mimicks the GFW by checking the URI field (naively) for the censored keyword.
"""
...
class Censor13(Censor):
    ...
    def check_censor(self, packet):
        ...
        try:
            ...
            # To defeat the evasion strategy that relies on corrupted checksums, 
            # we do not recalculate the checksum here
